...
export default function KairoSettingsModal({ isOpen, onClose, onKeySaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in select-none">
      <div className="w-full max-w-md bg-[#0f1728] border border-slate-700/80 rounded-3xl p-6 shadow-2xl space-y-5 text-slate-100 relative">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-xl neu-btn-raised text-slate-400 hover:text-white cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <form onSubmit={handlePinSubmit} className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="••••••"
                autoFocus
                className="w-full h-11 px-4 text-center tracking-[0.3em] font-mono text-base rounded-xl neu-inset text-cyan-300 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500"
              />
              {pinError && (
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 rounded-xl neu-btn-raised text-xs font-bold text-slate-400 hover:text-slate-200 cursor-pointer"
              >
                Cancel
              </button>
          ...
          <form onSubmit={handleTestAndSave} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-cyan-400 hover:text-cyan-300 flex items-center gap-1 px-2 py-0.5 rounded-lg neu-btn-raised cursor-pointer"
              ...
              <input
                ...
                autoComplete="off"
                className="w-full h-11 px-3.5 rounded-xl neu-inset text-xs font-mono text-cyan-300 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500"
              />
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              <button
                type="button"
                onClick={handleClear}
                className="text-xs text-slate-400 hover:text-rose-400 px-3 py-2 rounded-xl neu-btn-raised cursor-pointer"
              >
                Clear Key
              </button>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-2 rounded-xl neu-btn-raised text-xs font-bold text-slate-300 cursor-pointer"
                >
                  Close
                </button>
                <TouchButton
